...
class ReporteIncendioServiceTest {
    ...
    void testFindAll_Exitoso() {
        ...
        List<ReporteIncendio> resultados = reporteIncendioService.findAll();
    ...
    void testObtenerReportesPorEstado() {
        ...
        when(reporteIncendioRepository.obtenerReportesPorEstado()).thenReturn(Arrays.asList(row1, row2));

        List<Object[]> resultados = reporteIncendioService.obtenerReportesPorEstado();
        assertNotNull(resultados);
        assertEquals(2, resultados.size());
        assertEquals("PENDIENTE", resultados.get(0)[0]);
